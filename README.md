...
-- This table allows one user to have multiple devices
CREATE TABLE public.user_devices (
  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  user_id UUID REFERENCES auth.users(id) NOT NULL,
  mac_address TEXT NOT NULL,
  device_name TEXT DEFAULT 'Device 1',
  created_at TIMESTAMP DEFAULT now(),
...
CREATE INDEX idx_user_devices_mac_address ON public.user_devices(mac_address);
```
...
-- Trigger 2: Auto-Map Sensor Data to User (via user_devices table)
CREATE OR REPLACE FUNCTION public.link_user_to_sensor_data()
RETURNS TRIGGER AS $$
DECLARE
    owner_id UUID;
BEGIN
    -- First try to find owner in user_devices table (multi-device support)
    SELECT user_id INTO owner_id FROM public.user_devices WHERE mac_address = NEW.mac_address LIMIT 1;

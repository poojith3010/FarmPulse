...
# Import Config (Assuming these exist in your project)
...
async def lifespan(app: FastAPI):
    ...
    try:
        # Load the JSON rules you provided
    ...
    for rule in rules_data['rule_set']:
        ...
        if 'growth_stage_any_of' in conditions:
            if current_growth_stage not in conditions['growth_stage_any_of']: 
                # print(f"Skipping {rule_id}: Wrong Stage")
                continue 

        rule_triggered = False 
        
        # 3. Check Sensor Thresholds
        if 'sensor' in conditions:
            ...
            for metric, checks in sensor_conditions.items():
                if metric in sensor_readings:
                    ...
                    # DEBUG PRINT
                    print(f"Checking {rule_id}: {metric} = {actual_value} (Threshold: {checks})")

                    if 'less_than_or_equal_to' in checks and actual_value <= checks['less_than_or_equal_to']: 
                        rule_triggered = True
                    elif 'less_than' in checks and 'or_greater_than' not in checks and actual_value < checks['less_than']: 
                        rule_triggered = True
        ...
        if rule_triggered:
            print(f"🚨 ALERT TRIGGERED: {rule_id}")
            template = rule['action_template']
            
            # Safe parsing of stage info
            ...
            response = template.format(
                ...
                live_pH=sensor_readings.get('soil_pH', 'N/A'), 
                units="kg/ha", 
                target_N=targets.get('N', '?'), 
                target_P=targets.get('P2O5', '?'), 
                target_K=targets.get('K2O', '?'),
                target_units="kg/ha", 
                suggested_N_rate_kg_per_ha=100,  # You can make this dynamic later
...
def ask_plant_doctor(query: UserQuery):
    ...
    # --- CRITICAL FIX: FORCE FLOAT CONVERSION ---
    # This prevents "2.0" (string) <= 250 (int) errors
    try:
        mapped_readings = {
            "soil_nitrogen_ppm_or_kg_per_ha": float(clean_data['nitrogen']),
            "available_P2O5_kg_per_ha": float(clean_data['phosphorus']),
            "available_K2O_kg_per_ha": float(clean_data['potassium']),
            "soil_pH": float(clean_data['ph'])
        }
    except ValueError:
        return {"reply": "Error: Sensor data contains non-numeric values."}
    
    # Run the Rules (Using the hardcoded stage from your example)
    technical_advice = evaluate_nutrient_needs("high_demand_plucking_flush", mapped_readings, RULES)
    
    # 4. GENERATIVE REWRITE (SAFETY NET ADDED)
    system_prompt = (
        ...
        "3. SANITY CHECK (CRITICAL): If any nutrient value is extremely low (e.g., < 10 kg/ha for N/P/K), "
        "you MUST warn the user that the soil is depleted, even if it is NOT listed in 'ACTIVE ALERTS'."
        "4. If the nutrient is in a healthy range (> 10) AND not in 'ACTIVE ALERTS', explicitly state that it is HEALTHY and SAFE."
    ...
    - Nitrogen: {mapped_readings['soil_nitrogen_ppm_or_kg_per_ha']} kg/ha
    - Phosphorus: {mapped_readings['available_P2O5_kg_per_ha']} kg/ha
    - Potassium: {mapped_readings['available_K2O_kg_per_ha']} kg/ha
    - pH: {mapped_readings['soil_pH']}

    ACTIVE ALERTS (Problems detected by Logic Engine):
    {technical_advice if technical_advice else "None detected by rule engine."}
    """
